...
def replace_code(markdown_text, pattern, parent_folder=None, replacement_func=None):
    """
    ...
    def default_replacer(src, title, inner, description, thumbnail):
        ...
        # file_path = Path(src)
        if parent_folder:
            display_path = '/' + parent_folder + '/demos/' + src
        else:
            display_path = '/demos/' + src
    ...
    def match_handler(match):
        ...
        attr_matches = re.findall(r'(\w+)\s*=\s*([\'"])(.*?)\2', attrs_str)
        attrs = {key: value for key, quote, value in attr_matches}
    ...
    result = re.sub(pattern, match_handler, markdown_text, flags=re.DOTALL | re.IGNORECASE)
    return result
...
def get_description_and_when_to_use(md_content, when_to_use_prefixes=None):
    ...
    # 使用正则表达式匹配以 "# " 开头的标题行
    matches = re.findall(r'^#\s+(.+)', md_content, re.MULTILINE)
